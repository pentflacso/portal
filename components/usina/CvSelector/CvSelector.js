...
export default function CvSelector({ persons }) {
    ...
    const [dataSelector, setDataSelector] = useState(persons);
    const [personSelected, setPersonSelected] = useState(persons[0]?.nid);

    // Resetear el estado `personSelected` cada vez que cambian las `persons`
    useEffect(() => {
        setPersonSelected(persons[0]?.nid);
        setDataSelector(persons);
    }, [persons]);
                    ...
                    <div className={styles.cta_title}>
                        <div className={styles.wrapper}>
                            {persons.map((data, item) => {
                                return (
                                    <button
                                        onClick={() => setPersonSelected(`${data.nid}`)}
                                        className={personSelected === `${data.nid}` ? `${styles.active}` : `${styles.inactive}`}
                                        key={item}
                                    >
                                        <span />{data.nombre}
                                    </button>
                        ...
                        {dataSelector.map((data, item) => {
                            return (
                                <img
                                    src={data.img.url}
                                    alt={data.img.alt}
                                    className={personSelected === data.nid ? `${styles.visible} ${styles.slide_docente}` : ''}
                                    key={item}
                                />
                            );
                        })}
