...
export default function Card({category, lead , title, subtitle , description, hashtags, date, cta, height, layout, img, url}){
  ...
  const ComponentContainer= cta ? Fragment : Link
    ...
    >
      <div className={ 
        height ? height === "hidden" ? img ? `${styles.card} ${styles.height}` : `${styles.wrapper} ${styles.height}` : img ? styles.card : styles.wrapper : img ? styles.card : styles.wrapper}>

          {(layout == "imgTop" || !layout) && img ? <img src={ img } alt={ title } className={styles.imgTop} /> : ""}
